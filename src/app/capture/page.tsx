...
export default function CapturePage() {
  ...
  const [uploadResult, setUploadResult] = useState<{ sessionId: string; videoUrl: string; sensorUrl: string } | null>(null);
  ...
  // Load orgs and ensure consent is signed (redirect to onboarding if not)
  useEffect(() => {
    (async () => {
      ...
      // Load expert hardware tags for compatibility filtering
      ...
      const orgs = rows
        .map((r: { organization_id: string; organizations: { id: string; name: string } | { id: string; name: string }[] | null }) => {
  ...
  // Load tasks for the selected org
  useEffect(() => {
  ...
  const startRecording = useCallback(async () => {
    try {
      ...
      setErrorMessage(msg.includes("Permission") || msg.includes("NotAllowed") || msg.includes("denied")
      ...
      <div className="container max-w-lg mx-auto px-4 pt-6">
        ...
        <Card className="overflow-hidden">
          ...
          <CardContent className="space-y-4">
            {status === "uploading" && (
              <div className="rounded-lg border border-primary/30 bg-primary/5 p-5 flex flex-col items-center gap-3" role="status" aria-live="polite">
                ...
                <div className="w-full h-1.5 rounded-full bg-muted overflow-hidden">
                  <div className="h-full w-full rounded-full bg-primary/40 animate-pulse origin-left" style={{ animationDuration: "1.2s" }} />
                </div>
              </div>
            )}
            ...
            {!selectedTaskCompatible && (
              <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-3">
                <p className="text-sm font-medium text-amber-900 dark:text-amber-200">
                  This task requires hardware not in your profile.
                </p>
            ...
            {uploadResult && (
              <div className="rounded-lg border border-green-500/30 bg-green-500/5 p-4 space-y-2">
                <p className="text-sm font-medium text-green-700 dark:text-green-400 flex items-center gap-2">
                  <span className="h-5 w-5 rounded-full bg-green-500/20 flex items-center justify-center text-green-600 dark:text-green-400" aria-hidden>✓</span>
                  Upload complete
                </p>
                <p className="text-xs text-muted-foreground">Video and sensor JSON uploaded.</p>
                <p className="text-xs font-mono text-muted-foreground break-all" title="Session ID">{uploadResult.sessionId}</p>
              </div>
            )}
          </CardContent>
        </Card>
